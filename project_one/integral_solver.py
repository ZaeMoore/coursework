...
def riemann(step_size, radius, max_r):
    ...
    total_sum = 0
    while ri < radius:
        new_in = 4 * np.pi * step_size * rho(ri) #4pi is from the theta and phi portion
        total_sum += new_in
        solution_in.append(total_sum)
        r_in.append(ri)
        ri += step_size
    while ro < max_r: #Outside of the sphere, the enclosed charge Q is just the total charge of the sphere
